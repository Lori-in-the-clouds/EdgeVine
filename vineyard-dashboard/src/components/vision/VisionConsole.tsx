...
import { Camera, Trash2, Cpu, Leaf, CheckCircle, AlertTriangle, Layers, Radio, Save } from 'lucide-react';
    ...
    try {
      const response = await fetch('/api/vision/save-result', {
        ...
        body: JSON.stringify({
          ...
          estimated_liters: null, // No yield estimation in field-captured photos
          liters_min: null,
          liters_max: null,
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            <h2 className="text-3xl font-manrope font-black text-stone-800">Manual Field Capture</h2>
            <p className="text-stone-500 font-medium">Upload or snap high-resolution crop photos to analyze leaves and count bunches</p>
          </div>
      </div>
        ...
        >
          {!previewUrl ? (
            <div 
              ...
              className="flex flex-col items-center gap-6 cursor-pointer group-hover:scale-105 transition-all duration-500 animate-pulse"
            >
              ...
              <div className="text-center space-y-2">
                <p className="text-xl font-black text-stone-800">Capture or select field photo</p>
                <p className="text-xs font-bold text-stone-400 uppercase tracking-widest">TAP TO USE SMARTPHONE CAMERA OR DRAG IMAGE HERE</p>
              </div>
              ...
              {isAnalyzing && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-6">
                  ...
                  <div className="px-6 py-2 bg-stone-900 rounded-full">
                    <span className="text-xs font-black text-[#228B22] uppercase tracking-[0.3em] animate-pulse">Running Crop Scan Inference...</span>
                  </div>
                </div>
              )}
          ...
          <div className="flex-1 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-[#228B22] mt-0.5">
                  Please select the Sentinel node closest to where you captured this photo.
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className={`p-8 rounded-[2rem] border border-white/5 transition-all duration-700 ${result ? 'bg-white/5 transform translate-y-0 opacity-100' : 'opacity-20 translate-y-4'}`}>
               <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-[10px] font-black text-stone-500 uppercase tracking-widest">Canopy Health & Leaves</span>
               </div>
               <div className="grid grid-cols-3 gap-2">
                  <div className="bg-stone-950/40 p-4 rounded-xl border border-white/5 flex flex-col items-center justify-center text-center">
                    <span className="text-[9px] font-bold text-stone-500 uppercase tracking-wider mb-1">Healthy</span>
                    <span className="text-xl font-black text-emerald-400">{result ? (result.leaf_healthy_count ?? 0) : '--'}</span>
                  </div>
                  <div className="bg-stone-950/40 p-4 rounded-xl border border-white/5 flex flex-col items-center justify-center text-center">
                    <span className="text-[9px] font-bold text-stone-500 uppercase tracking-wider mb-1">Stressed</span>
                    <span className="text-xl font-black text-amber-500">{result ? (result.leaf_stress_count ?? 0) : '--'}</span>
                  </div>
                  <div className="bg-stone-950/40 p-4 rounded-xl border border-white/5 flex flex-col items-center justify-center text-center">
                    <span className="text-[9px] font-bold text-stone-500 uppercase tracking-wider mb-1">Diseased</span>
                    <span className="text-xl font-black text-red-500">{result ? (result.leaf_disease_count ?? 0) : '--'}</span>
                  </div>
               </div>
            </div>
            ...
            <div className={`p-8 rounded-[2rem] border border-white/5 transition-all duration-700 delay-100 ${result ? 'bg-white/5 transform translate-y-0 opacity-100' : 'opacity-20 translate-y-4'}`}>
               ...
               <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-xs font-bold text-stone-500 uppercase tracking-widest font-manrope">Grapes</span>
               </div>
            </div>

            {/* Metric: Diseases / Overall Health */}
            <div className={`p-8 rounded-[2rem] border border-white/5 transition-all duration-700 delay-200 ${result ? 'bg-white/5 transform translate-y-0 opacity-100' : 'opacity-20 translate-y-4'}`}>
               <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-[10px] font-black text-stone-500 uppercase tracking-widest">Overall Health Rating</span>
               </div>
               ...
               {hasHealthAlert ? (
                  ...
                  <span className="text-xl font-black text-white tracking-tight">{result ? 'Optimal (Healthy)' : 'Ready to scan'}</span>
               )}
            </div>
