...
export function DashboardStats() {
  ...
  useEffect(() => {
    ...
    // Fetch Current Data (for UI visuals)
    fetch('/api/sensors')
      .then(r => r.json())
      .then(d => {
        if (d.success && d.data?.length > 0) {
          // Mocking AI Vision stats based on mockup
          setLatestStats({
            safe: 78,
            stress: 15,
            disease: 7,
  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Canopy Health Diagnostics Block */}
        ...
        <div className="bg-[#228B22] rounded-[2.5rem] p-8 shadow-2xl shadow-green-900/20 text-white flex flex-col relative overflow-hidden group">
          ...
          <div className="relative z-10 flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-manrope font-black">Wine Yield Estimation</h2>
            </div>
      ...
      {/* SECTION 2: CORE TELEMETRY KPIs */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="vineyard-card flex items-center gap-6 p-8">
          <div className="w-16 h-16 bg-[#228B22]/10 rounded-full flex items-center justify-center">
            <Thermometer className="h-8 w-8 text-[#228B22]" />
          </div>
          <div className="flex flex-col">
        ...
        <div className="vineyard-card flex items-center gap-6 p-8">
          <div className="w-16 h-16 bg-[#228B22]/10 rounded-full flex items-center justify-center">
            <Droplets className="h-8 w-8 text-[#228B22]" />
          </div>
          <div className="flex flex-col">
        ...
        <div className="vineyard-card flex items-center gap-6 p-8">
          <div className="w-16 h-16 bg-[#228B22]/10 rounded-full flex items-center justify-center">
            <Sprout className="h-8 w-8 text-[#228B22]" />
          </div>
          <div className="flex flex-col">
      ...
      <div className="vineyard-card p-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          ...
          <div className="flex items-center gap-3 bg-stone-100 rounded-3xl p-1.5 shadow-inner">
            <div className="flex items-center px-4 gap-2">
              ...
              <span className="text-[10px] font-black text-stone-500 uppercase tracking-widest">Show</span>
            </div>
            <div className="flex gap-1">
              {[5, 10, 15, 20].map(num => (
                <button
                  key={num}
                  onClick={() => setImageLimit(num)}
                  className={`px-5 py-2.5 rounded-2xl text-[10px] font-black transition-all duration-300 ${
                    imageLimit === num 
                      ? 'bg-white text-[#228B22] shadow-xl scale-105' 
            ...
            <div key={img.id} className="group flex flex-col bg-white rounded-[2rem] overflow-hidden border border-stone-100 shadow-sm hover:shadow-2xl transition-all duration-700 hover:-translate-y-2">
              <div className="relative aspect-[4/5] overflow-hidden">
                ...
                <div className="absolute top-4 left-4 px-4 py-1.5 bg-black/40 backdrop-blur-md rounded-full border border-white/20">
                  <p className="text-[9px] font-black text-white uppercase tracking-[0.2em]">{img.sensorName}</p>
                </div>
              </div>
              <div className="p-6 bg-white">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-black text-on-surface font-manrope uppercase tracking-tight">{img.date}</p>
                  <p className="text-[10px] font-bold text-stone-400 font-inter">{img.time}</p>
                </div>
              </div>
            </div>
          ))}
